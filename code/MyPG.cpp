...
namespace MyPG
{
	...
	MyGameEngine::MyGameEngine()
		:GameEngine(
			//	環境情報
			GameDefine::SCREEN_W,							//	画面サイズX
			GameDefine::SCREEN_H,						//	画面サイズY
			1,								//	表示倍率
			1,								//	マルチサンプル(1or2or4)
			false,							//	フルスクリーンモード
			GameDefine::SCREEN_W,							//	2D描画用解像度X(現在非対応）
			GameDefine::SCREEN_H,							//	2D描画用解像度Y(現在非対応）
			"異世界パン屋",		//	ウインドウタイトル
			"ISEKAI_BAKERY",							//	ウィンドウクラス名
	...
	bool MyGameEngine::Initialize(HWND wnd_)
	{
		...
		this->camera[0] = MyPG::Camera::Create(
			...
			ML::ToRadian(60.0f), 10.0f, 300.0f,	//	隕夜㍽隗偵・隕夜㍽霍晞屬
